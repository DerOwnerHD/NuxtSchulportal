    ...
    <div class="h-full py-8">
        ...
        <div v-else class="px-2 grid gap-2">
            <section
                ...
                @click="openIndex = index === openIndex ? -1 : index"
                class="status blurred-background borderless rounded-3xl p-2"
                :class="{ unavailable: item.unavailable, update: item.update_planned }">
                <header class="flex gap-2 items-center w-full">
                    <!-- See https://github.com/microsoft/fluentui-emoji for this icon (MIT license) -->
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 8 32 14" fill="none" class="w-40 min-w-40">
                    ...
                    <div class="flex gap-2 items-center justify-between w-full">
                        ...
                        <ButtonRoundedBlurred :icon="['fas', index === openIndex ? 'chevron-up' : 'chevron-down']"></ButtonRoundedBlurred>
                    </div>
                </header>
                <main v-if="index === openIndex" class="px-2">
                    <section>
                        <h2>Status</h2>
                        <p v-html="item.status_string"></p>
                    </section>
                    <section>
                        <h2>Update</h2>
                        <p v-html="item.update_string"></p>
                    </section>
                </main>
            </section>
            <ButtonDefault
...
import type { SPHStatus } from "~/common";
const STATUS_PAGE_URL = "https://info.schulportal.hessen.de/status-des-schulportal-hessen/";
const openIndex = ref(-1);
const status = ref<SPHStatus[] | null>(null);
const appErrors = useAppErrors();
async function fetchStatus() {
    status.value = null;
    appErrors.value.delete(AppID.Status);
    try {
        const data = await $fetch("/api/sph-status");
        if (data.error) throw { data };
        status.value = data.items;
    } catch (error) {
        // @ts-ignore
        appErrors.value.set(AppID.Status, error?.data?.error_details ?? error);
    }
}
...
.status:not(.update) .traffic-light-yellow {
    opacity: var(--opacity);
}
.status:not(.unavailable) .traffic-light-red {
    opacity: var(--opacity);
}
.status.unavailable .traffic-light-green {
    opacity: var(--opacity);
}
</style>
