...
export const start = (operation: string): string => {
    ...
    if (debug) console.info(`[Metrics] Session of operation ${operation} started`);
...
export const end = (uuid: string): any => {
    ...
    if (!session) return console.warn("[Metrics] ⚠ No Metric Session found for a given UUID");

    const { operation, start } = session;
    const timeElapsed = performance.now() - start;
    if (debug) console.info(`[Metrics] Session of operation ${operation} ended after ${timeElapsed}ms`);
    if (!operations[operation]) return (operations[operation] = { count: 1, median: timeElapsed, total: timeElapsed });
    ...
    if (debug) console.info(`[Metrics] Median of operation ${operation}: ${median}ms => ${operations[operation].median}ms`);
};
